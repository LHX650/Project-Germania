"""Local HTML parser for Audi Germany official prices."""
...
from germania.collectors.audi.config import AUDI_DE_BASE_URL, AUDI_DE_SOURCE_ID
...
class AudiOfficialPriceParser:
    """Parse local Audi Germany HTML into official price records."""
...
def parse_audi_official_prices(
    html: str,
    *,
    source_url: str | None = None,
    collected_at: datetime | None = None,
) -> list[OfficialPriceRecord]:
    """Parse official Audi price records from a local HTML page string."""

    return AudiOfficialPriceParser().parse_price_page(
        html,
...
def _parse_html(html: str) -> _Node:
    ...
    try:
        ...
        logger.warning(
            "Audi HTML parsing recovered from malformed input: %s",
            exc,
        )
    return parser.root
...
def _node_source_url(node: _Node) -> str | None:
    ...
    if source_url:
        return urljoin(AUDI_DE_BASE_URL, source_url)
    ...
    return urljoin(AUDI_DE_BASE_URL, href) if href else None
