"""Local HTML parser for Skoda Germany official prices."""
...
from germania.collectors.skoda.config import SKODA_DE_BASE_URL, SKODA_DE_SOURCE_ID
...
class SkodaOfficialPriceParser:
    """Parse local Skoda Germany HTML into official price records."""
...
def parse_skoda_official_prices(
    html: str,
    *,
    source_url: str | None = None,
    collected_at: datetime | None = None,
) -> list[OfficialPriceRecord]:
    """Parse official Skoda price records from a local HTML page string."""

    return SkodaOfficialPriceParser().parse_price_page(
        html,
...
def _parse_html(html: str) -> _Node:
    ...
    try:
        ...
        logger.warning(
            "Skoda HTML parsing recovered from malformed input: %s",
            exc,
        )
    return parser.root
...
def _node_source_url(node: _Node) -> str | None:
    ...
    if source_url:
        return urljoin(SKODA_DE_BASE_URL, source_url)
    ...
    return urljoin(SKODA_DE_BASE_URL, href) if href else None
