"""Local HTML parser for Mercedes-Benz Germany official prices."""
...
class MercedesBenzOfficialPriceParser:
    """Parse local Mercedes-Benz Germany HTML into official price records."""
...
def parse_mercedes_benz_official_prices(
    html: str,
    *,
    source_url: str | None = None,
    collected_at: datetime | None = None,
) -> list[OfficialPriceRecord]:
    """Parse official Mercedes-Benz price records from a local HTML page string."""

    return MercedesBenzOfficialPriceParser().parse_price_page(
        html,
...
def _parse_html(html: str) -> _Node:
    ...
    try:
        ...
        logger.warning(
            "Mercedes-Benz HTML parsing recovered from malformed input: %s",
            exc,
        )
    return parser.root
...
def _record_from_node(
    ...
    *,
    ...
    return OfficialPriceRecord(
        ...
        collected_at=collected_at,
        source_id=MERCEDES_BENZ_DE_SOURCE_ID,
...
def _node_source_url(node: _Node) -> str | None:
    ...
    if source_url:
        return urljoin(MERCEDES_BENZ_DE_BASE_URL, source_url)
    ...
    return urljoin(MERCEDES_BENZ_DE_BASE_URL, href) if href else None
