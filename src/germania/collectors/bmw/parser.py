"""Local HTML parser for BMW Germany official prices."""
...
from germania.collectors.bmw.config import BMW_DE_BASE_URL, BMW_DE_SOURCE_ID
...
class BMWOfficialPriceParser:
    """Parse local BMW Germany HTML into official price records."""
...
def parse_bmw_official_prices(
    html: str,
    *,
    source_url: str | None = None,
    collected_at: datetime | None = None,
) -> list[OfficialPriceRecord]:
    """Parse official BMW price records from a local HTML page string."""

    return BMWOfficialPriceParser().parse_price_page(
        html,
...
def _parse_html(html: str) -> _Node:
    ...
    try:
        ...
        logger.warning(
            "BMW HTML parsing recovered from malformed input: %s",
            exc,
        )
    return parser.root
...
def _node_source_url(node: _Node) -> str | None:
    ...
    if source_url:
        return urljoin(BMW_DE_BASE_URL, source_url)
    ...
    return urljoin(BMW_DE_BASE_URL, href) if href else None
