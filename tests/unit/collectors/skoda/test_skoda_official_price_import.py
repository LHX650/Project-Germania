...
from germania.collectors.skoda import (
    OfficialPriceRecord,
...
FIXTURE_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "skoda"
ENYAQ_PRICE_FIXTURE = FIXTURE_DIR / "enyaq_official_prices.html"
COLLECTED_AT = datetime(2026, 7, 18, 10, 30, tzinfo=UTC)
SOURCE_URL = "https://www.skoda-auto.de/modelle/enyaq/enyaq"


def test_parser_reads_enyaq_official_starting_price_fixture() -> None:
    records = parse_skoda_official_prices(
        _fixture_text("enyaq_official_prices.html"),
    ...
    assert record.source_id == "skoda_de"
    assert record.source_url == SOURCE_URL
    assert record.raw_brand_name == "Skoda"
    assert record.raw_model_name == "Enyaq"
    assert record.raw_variant_name == "Enyaq 60"
    assert record.canonical_brand_name == "Škoda"
    assert record.canonical_model_name == "Enyaq"
    assert record.price_type == "official_starting_price"
    assert record.original_price == Decimal("44900.00")
    assert isinstance(record.original_price, Decimal)
    assert record.original_currency == "EUR"
    assert record.price_eur == Decimal("44900.00")
    assert record.raw_price_text == "ab 44.900,00 EUR"
...
def test_parser_parses_german_price_format_as_decimal() -> None:
    record = parse_skoda_official_prices(
        _official_price_html("44.900,00 EUR"),
        collected_at=COLLECTED_AT,
    )[0]
...
def test_parser_does_not_treat_monthly_payments_as_official_prices() -> None:
    records = SkodaOfficialPriceParser().parse_price_page(
        _fixture_text("enyaq_official_prices.html"),
        collected_at=COLLECTED_AT,
    )
    ...
    assert Decimal("389.00") not in {record.price_eur for record in records}
    assert Decimal("489.00") not in {record.price_eur for record in records}
...
def test_import_service_rejects_missing_price(
    ...
    with session_scope(db_sessions) as session:
        ...
        result = SkodaOfficialPriceImportService(session).import_records(
            parse_skoda_official_prices(
...
def test_import_service_first_import_writes_official_price(
    ...
    with session_scope(db_sessions) as session:
        _seed_official_price_master_data(session)
        result = SkodaOfficialPriceImportService(session).import_html(
            ENYAQ_PRICE_FIXTURE,
            collected_at=COLLECTED_AT,
        )
        ...
        assert observation.official_price == Decimal("44900.00")
        assert observation.currency == "EUR"
        assert observation.valid_date == date(2026, 7, 18)
        assert observation.source_url == SOURCE_URL
        assert observation.raw_brand == "Skoda"
        assert observation.raw_model == "Enyaq"
        assert observation.raw_variant_name == "Enyaq 60"
...
def test_import_service_second_import_is_idempotent(
    ...
    with session_scope(db_sessions) as session:
        _seed_official_price_master_data(session)
        service = SkodaOfficialPriceImportService(session)
...
def test_import_service_unknown_vehicle_does_not_create_master_data(
    ...
    with session_scope(db_sessions) as session:
        ...
        result = SkodaOfficialPriceImportService(session).import_records(
            parse_skoda_official_prices(
                _official_price_html("37.490,00 EUR", model="Kodiaq"),
                collected_at=COLLECTED_AT,
            )
        )
...
def test_import_writes_to_official_price_observations_table(
    ...
    with session_scope(db_sessions) as session:
        _seed_official_price_master_data(session)
        SkodaOfficialPriceImportService(session).import_html(
            ENYAQ_PRICE_FIXTURE,
            collected_at=COLLECTED_AT,
        )
...
def _official_price_html(price_text: str, *, model: str = "Enyaq") -> str:
    return f"""
    <article data-price-card data-price-type="official_starting_price">
      <span data-field="brand">Skoda</span>
      <span data-field="model">{model}</span>
      <span data-field="variant">Enyaq 60</span>
      <span data-field="canonical_brand">Škoda</span>
      <span data-field="canonical_model">{model}</span>
...
def _seed_official_price_master_data(session: Session) -> VehicleVariant:
    DataSourceRepository(session).add(
        DataSource(
            source_id="skoda_de",
            source_name="Skoda Germany",
            source_type="manufacturer",
            country_code="DE",
            base_url="https://www.skoda-auto.de",
            update_frequency="irregular",
            authority_level="primary_commercial",
            active=True,
            collection_method="html_parse",
            notes="Test Skoda official price source.",
        )
    )
    brand = BrandRepository(session).add(
        Brand(
            canonical_brand="Škoda",
            chinese_brand=None,
    ...
    vehicle = VehicleRepository(session).add(
        Vehicle(
            ...
            canonical_model="Enyaq",
            chinese_model=None,
            vehicle_segment="midsize_electric_suv",
            body_type="suv",
